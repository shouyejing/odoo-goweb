    ...
    def action_ship_create(self, cr, uid, ids, context=None):
        ...
        for order in self.browse(cr, uid, ids, context=context):
            ...
            for procurement in procurement_obj.browse(cr,uid, proc_ids,context):
                partner = procurement_obj._get_product_supplier(cr, uid, procurement, context=context)
                available_draft_po_ids = po_obj.search(cr, uid, [
                        ('partner_id', '=', partner.id), ('state', '=', 'draft')], context=context)
                for po_id in available_draft_po_ids:
                    if po_id not in found_po:
                        found_po.append(po_id)
            for po in found_po:
                workflow.trg_validate(1, 'purchase.order', po, 'purchase_confirm', cr)
