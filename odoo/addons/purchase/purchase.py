...
class procurement_order(osv.osv):
    ...
    def make_po(self, cr, uid, ids, context=None):
        ...
        #pdb.set_trace()
        #draft_po_ids = po_obj.search(cr, uid, [('state','=', 'draft')])
        #draft_po = po_obj.browse(cr, uid, draft_po_ids)
        #for po in draft_po:
        #    workflow.trg_validate(SUPERUSER_ID, 'purchase.order', po.id, 'wkf_confirm_order', cr)
            #workflow.trg_validate(SUPERUSER_ID, 'purchase.order', po.id, 'wkf_approve_order', cr)
        if pass_ids:
