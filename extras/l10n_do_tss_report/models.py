...
class tss_report(osv.Model):
    ...
    def create_tss_report_line(self, cr, uid, tss_report_id, period_id, context=None):
        ...
        for line in key_output:
            ...
            for employee_id in output:
                ...
                if not employee_id.identification_id and not employee_id.passport_id:
                ...
                comisions_codes = ['COM2']
                slip_comisions_ids = hr_payslip_line_obj.search(cr, uid, [("slip_id", "in", payslip_ids),
                    ('employee_id', '=', employee_id.id), ("code","in",comisions_codes), ("amount",">",0)])
                for slip_comisions_line in hr_payslip_line_obj.browse(cr, uid, slip_comisions_ids):
                    comisions += slip_comisions_line.amount

                holidays_codes = ['VAC','VAC1','VAC2','VAC3']
                ...
                salario_isr = (basic - tss_amount) + (overtime + holidays + comisions)
    ...
    def action_generate_txt(self, cr, uid, ids, context=None):
        ...
        tipo_registro = str('S')
        numero_registro = str(counter).zfill(6)
