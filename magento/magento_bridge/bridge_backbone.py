...
class bridge_backbone(models.Model):
    ...
    def create_pricelist(self, cr, uid, data, context=None):
        ...
        currency_ids = self.pool.get('res.currency').search(
            cr, uid, [('name', '=', data['code'])]) or [False]
        if currency_ids:
            pricelist_ids = self.pool.get('product.pricelist').search(
                cr, uid, [('currency_id', '=', currency_ids[0])], context=context) or [False]
            if pricelist_ids[0] == False:
                ...
                pricelist_id = self.pool.get("product.pricelist").create(
                    cr, uid, pricelist_dict, context)
                version_dict = {
                    'name': data['code'] + ' Public Pricelist Version',
                    'pricelist_id': pricelist_id,
                    'active': True,
                }
                product_version_id = self.pool.get(
                    'product.pricelist.version').create(cr, uid, version_dict, context)
                price_type_id = self.pool.get('product.price.type').search(
                ...
                product_pricelist_item_id = self.pool.get(
                    'product.pricelist.item').create(cr, uid, item_dict, context)
                return pricelist_id
            else:
    ...
    def _get_journal_code(self, cr, uid, string, sep=' ', context=None):
        ...
        code = ''.join(tl)
        code = code[0:3]
        is_exist = self.pool.get('account.journal').search(
            cr, uid, [('code', '=', code)], context=context)
        if is_exist:
            for i in range(1, 99):
                is_exist = self.pool.get('account.journal').search(
                    cr, uid, [('code', '=', code + str(i))], context=context)
                if not is_exist:
                    return code + str(i)[-5:]
        return code
    ...
    def create_payment_method(self, cr, uid, data, context=None):
        ...
        res = self.pool.get('account.journal').search(
            cr, uid, [('type', '=', 'cash')], limit=1, context=context)
        if res:
            credit_account_id = self.pool.get('account.journal').browse(
                cr, uid, res[0], context).default_credit_account_id.id
            debit_account_id = self.pool.get('account.journal').browse(
                cr, uid, res[0], context).default_debit_account_id.id
            data['default_credit_account_id'] = credit_account_id
            data['default_debit_account_id'] = debit_account_id
            data['code'] = self._get_journal_code(
                cr, uid, data.get('name'), ' ', context)
            payment_id = self.pool.get(
                'account.journal').create(cr, uid, data, context)
        return payment_id
    ...
    def _get_virtual_product_id(self, cr, uid, data):
        ...
        if data['name'].startswith('S'):
            erp_product_id = ir_values.get_default(
                cr, uid, 'product.product', 'mob_delivery_product')
        if data['name'].startswith('D'):
            erp_product_id = ir_values.get_default(
                cr, uid, 'product.product', 'mob_discount_product')
        if data['name'].startswith('V'):
            erp_product_id = ir_values.get_default(
                cr, uid, 'product.product', 'mob_coupon_product')
        if data['name'].startswith('C'):
            erp_product_id = ir_values.get_default(
                cr, uid, 'product.product', 'mob_credit_product')
        if not erp_product_id:
            ...
            if data['name'].startswith('S'):
                ...
                temp_dic[
                    'description'] = 'Service Type product used by Magento Odoo Bridge for Shipping Purposes'
            if data['name'].startswith('D'):
            ...
            if data['name'].startswith('C'):
                object_name = 'mob_credit_product'
                temp_dic[
                    'description'] = 'Service Type product used by Magento Odoo Bridge for Gift Wrap Cost'
            erp_product_id = self.pool.get(
                'product.product').create(cr, uid, temp_dic)
            ir_values.set_default(cr, uid, 'product.product',
                                  object_name, erp_product_id)
            cr.commit()
    ...
    def extra_order_line(self, cr, uid, data, context=None):
        ...
        if context.has_key('instance_id') and data['name'].startswith('S'):
            ...
            self.pool.get('sale.order').write(
                cr, uid, data['order_id'], {'consignee_id': consignee_id})
        ...
        line_dic['product_uom'] = self.pool.get(
            'product.product').browse(cr, uid, product_id).uom_id.id or 1
    ...
    def sales_reorder(self, cr, uid, order_id, context=None):
        ...
        """
        context = dict(context or {})
        reorder_id = self.pool.get('sale.order').copy_quotation(
            cr, uid, [order_id])['res_id']
        if reorder_id:
            self.pool.get('sale.order').signal_workflow(
                cr, uid, [reorder_id], 'order_confirm')
        return reorder_id
    ...
    def order_cancel(self, cr, uid, order_id, context=None):
        ...
        if context.has_key('instance_id'):
            ...
            state = self.pool.get('magento.configure').browse(
                cr, uid, active_id).state
            if state == 'enable':
                self.pool.get('magento.configure').write(
                    cr, uid, active_id, {'state': 'disable'})
            if pick_id:
                pick_cancel = self.pool.get(
                    'stock.picking').action_cancel(cr, uid, pick_id)
            order_cancel = self.pool.get(
                'sale.order').action_cancel(cr, uid, [order_id])
            if state == 'enable':
                self.pool.get('magento.configure').write(
                    cr, uid, active_id, {'state': 'enable'})
            return True
        return False
    ...
    def create_order_invoice(self, cr, uid, data, context=None):
        ...
        if data.get('order_id'):
            ...
            order_state = self.pool.get(
                'sale.order').browse(cr, uid, order_id).state
            ...
            if context.has_key('instance_id'):
                ...
                if state == 'enable':
                    self.pool.get('magento.configure').write(
                        cr, uid, active_id, {'state': 'disable'})
                sale_order_details = self.pool.get('sale.order').read(
                    cr, uid, data.get('order_id'), ['order_policy', 'invoice_ids'])
                if sale_order_details.get('order_policy') == 'prepaid':
                    ...
                    inv_ids = self.pool.get('sale.order').manual_invoice(
                        cr, uid, [data.get('order_id')])
                    invoice_id = inv_ids['res_id']
                if invoice_id:
                    if data.has_key('date'):
                        self.pool.get('account.invoice').write(cr, uid, invoice_id, {'date_invoice': data.get(
                            'date'), 'date_due': data.get('date'), 'internal_number': data['mage_inv_number']})
                    self.pool.get('account.invoice').signal_workflow(
                        cr, uid, [invoice_id], 'invoice_open')
                if state == 'enable':
                    self.pool.get('magento.configure').write(
                        cr, uid, active_id, {'state': 'enable'})
                workflow.trg_validate(uid, 'sale.order', data.get(
                    'order_id'), 'invoice_end', cr)
        return invoice_id
    ...
    def sales_order_payment(self, cr, uid, payment, context=None):
        ...
        data = voucher_obj.onchange_partner_id(cr, uid, [], partner_id, journal_id, int(
            amount), currency_id1, 'receipt', date, context)['value']
    ...
    def order_shipped(self, cr, uid, data, context=None):
        ...
        if context.has_key('instance_id'):
            ...
            if state == 'enable':
                self.pool.get('magento.configure').write(
                    cr, uid, active_id, {'state': 'disable'})
            order_name = self.pool.get('sale.order').name_get(
                cr, uid, data['order_id'])
            pick_ids = order_obj.picking_ids
            for pick_obj in pick_ids:
                status = pick_obj.state
                if status == "done":
                    continue
                self.pool.get('stock.picking').do_transfer(
                    cr, uid, pick_obj.id, context)
                self.pool.get('stock.picking').write(cr, uid, pick_obj.id, {'carrier_tracking_ref': data[
                    'carrier_tracking_ref'], 'carrier_code': data['carrier_code'], 'magento_shipment': data['mage_ship_number']}, context)
            workflow.trg_validate(uid, 'sale.order', data[
                                  'order_id'], 'ship_end', cr)
            res = True
            if state == 'enable':
                self.pool.get('magento.configure').write(
                    cr, uid, active_id, {'state': 'enable'})
    ...
    def update_quantity(self, cr, uid, data, context=None):
        ...
        if context.has_key('instance_id'):
            ...
            config_obj = self.pool.get(
                'magento.configure').browse(cr, uid, config_ids)
            active = config_obj.active
            context['warehouse'] = config_obj.warehouse_id.id
            res_original = prod_obj_pool.browse(
                cr, uid, rec_id, context=context)
            if active:
                warehouse_id = self.pool.get('magento.configure').browse(
                    cr, uid, config_ids).warehouse_id.id
                location_id = self.pool.get('stock.warehouse').browse(
                    cr, uid, warehouse_id).lot_stock_id.id
            else:
                location_ids = self.pool.get('stock.warehouse').search(
                    cr, uid, [], context=context)
                if location_ids:
                    location_id = self.pool.get('stock.warehouse').browse(
                        cr, uid, location_ids[0]).lot_stock_id.id
            if int(mage_qty) == res_original.qty_available:
                ...
                dest_location_id = self.pool.get('stock.location').search(
                    cr, uid, [('usage', '=', 'customer')], context=context)[0]
                line_data = {
                ...
                inventory_id = inventory_obj.create(cr, uid, {'name': _(
                    'INV: %s') % tools.ustr(res_original.name)}, context=context)
                line_data = {
