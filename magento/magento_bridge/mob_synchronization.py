...
class magento_synchronization(osv.osv):
    ...
    def export_attributes_and_their_values(self, cr, uid, ids, context=None):
        ...
        if connection:
            ...
            map_id = attribute_mapping_pool.search(
                cr, uid, [('instance_id', '=', context.get('instance_id'))])
            for m in map_id:
                ...
                for attribute_id in attribute_ids:
                    ...
                    if attribute_id not in map_dic:
                        ...
                        attribute_response = self.create_product_attribute(
                            cr, uid, url, session, attribute_id, name, label, context)
                    else:
                    ...
                    if attribute_response[0] == 0:
                        ...
                        for value_id in attribute_obj.value_ids:
                            if not value_mapping_pool.search(cr, uid, [('erp_id', '=', value_id.id), ('instance_id', '=', context.get('instance_id'))]):
                                value_obj = attribute_value_pool.browse(
                                    cr, uid, value_id.id, context)
                                name = value_obj.name
                                position = value_obj.sequence
                                value_response = self.create_attribute_value(
                                    cr, uid, url, session, mage_id, value_id.id, name, position, context)
    ...
    def create_product_attribute(self, cr, uid, url, session, attribute_id, name, label, context=None):
        ...
        if session:
            ...
            try:
                mage_id = server.call(
                    session, 'product_attribute.create', [attrribute_data])
    ...
    def create_attribute_value(self, cr, uid, url, session, mage_attr_id, erp_attr_id, name, position='0', context=None):
        ...
        if session:
            ...
            try:
                mage_id = server.call(session, 'product_attribute.addOption', [
                                      mage_attr_id, options_data])
            except xmlrpclib.Fault, e:
                return [0, ' Error in creating Option( %s ).%s' % (name, str(e))]
            if mage_id:
                self.pool.get('magento.product.attribute.value').create(cr, uid, {
                    'name': erp_attr_id, 'erp_id': erp_attr_id, 'mage_id': mage_id, 'instance_id': context.get('instance_id'), })
            ...
            for temp_id in template_ids:
                ...
                if attribute_line_ids:
                    for attr in attribute_line_ids:
                        set_search2 = []
                        set_search2 = mage_set_pool.search(
                            cr, uid, [('attribute_ids', '=', attr.attribute_id.id)])
                        if not set_search2:
                            set_search1 = []
                            break
                        if not set_search1:
                            ...
                            set_search1 = list(
                                set(set_search1) & set(set_search2))
                    if set_search1:
                        temp_pool.write(cr, uid, temp_id, {
                                        'attribute_set_id': set_search1[0]}, context)
                        success_ids.append(temp_id)
                    else:
                        fails_ids.append(temp_id)
                else:
                    temp_pool.write(cr, uid, temp_id, {
                                    'attribute_set_id': default_set}, context)
                    success_ids.append(temp_id)
        if success_ids:
    ...
    def export_categories(self, cr, uid, ids, context=None):
        ...
        if connection:
            ...
            erp_catg = self.pool.get('product.category').search(
                cr, uid, [('id', 'not in', map_dic)], context=context)
            if not erp_catg:
                raise osv.except_osv(_('Information'), _(
    ...
    def sync_categories(self, cr, uid, url, session, erp_category_ids, context=None):
        context = dict(context or {})
        category_ids = []

        if isinstance(erp_category_ids, (int, long)):
        ...
        for erp_category_id in erp_category_ids:
            ...
            if obj_catg.parent_id.id:
                p_cat_id = self.sync_categories(
                    cr, uid, url, session, obj_catg.parent_id.id, context)
            else:
                p_cat_id = self.create_category(
                    cr, uid, url, session, erp_category_id, 1, name, context)
                return p_cat_id
            category_id = self.create_category(
                cr, uid, url, session, erp_category_id, p_cat_id, name, context)
    ...
    def create_category(self, cr, uid, url, session, catg_id, parent_id, catgname, context=None):
        ...
        if context.has_key('instance_id'):
            ...
            if connection:
                instance_id = connection[2]
        map_category_id = map_category_obj.search(
            cr, uid, [('oe_category_id', '=', catg_id), ('instance_id', '=', instance_id)])

        if not map_category_id:
            catgdetail = dict({
                'name': catgname,
                'is_active': 1,
                'available_sort_by': 1,
            ...
            try:
                mage_cat = server.call(
                    session, 'catalog_category.create', updatecatg)
            except xmlrpclib.Fault, e:
                return 0
        else:
            mage_cat = map_category_obj.browse(
                cr, uid, map_category_id[0], context=context).mag_category_id
            return mage_cat
        if mage_cat > 0:
            ##########  Mapping Entry  ###########
            map_category_obj.create(cr, uid, {'cat_name': catg_id, 'oe_category_id': catg_id,
                                              'mag_category_id': mage_cat, 'created_by': 'odoo', 'instance_id': context.get('instance_id')})
            server.call(session, 'magerpsync.category_map', [
                        {'mage_category_id': mage_cat, 'erp_category_id': catg_id}])
            return mage_cat
    ...
    def update_categories(self, cr, uid, ids, context=None):
        ...
        if connection:
            ...
            instance_id = connection[2]
            server = xmlrpclib.Server(url)
            map_id = self.pool.get('magento.category').search(cr, uid, [(
                'need_sync', '=', 'Yes'), ('mag_category_id', '!=', -1), ('instance_id', '=', instance_id)])

            if not map_id:
                raise osv.except_osv(_('Information'), _(
                    "No category(s) has been found to be Update on Magento!!!"))
            if map_id:
                for i in map_id:
                    cat_update = self._update_specific_category(
                        cr, uid, i, url, session, context)
                    if cat_update:
                        if cat_update[0] != 0:
                ...
                if success_ids:
                    ...
                    self.pool.get('magento.sync.history').create(cr, uid, {
                        'status': 'yes', 'action_on': 'category', 'action': 'c', 'error_message': text})
                if up_error_ids:
                    text1 = 'The Listed Category ids %s does not updated on Magento.' % up_error_ids
                    self.pool.get('magento.sync.history').create(cr, uid, {
                        'status': 'no', 'action_on': 'category', 'action': 'c', 'error_message': text1})
                partial = self.pool.get('message.wizard').create(
                    cr, uid, {'text': text + text1})
                return {'name': _("Information"),
                        'view_mode': 'form',
                        'view_id': False,
                        'view_type': 'form',
    ...
    def export_bulk_category(self, cr, uid, context=None):
        ...
        if connection:
            ...
            context['instance_id'] = connection[2]
            for l in bulk_ids:
                search = map_pool.search(
                    cr, uid, [('cat_name', '=', l), ('instance_id', '=', context.get('instance_id'))])
                if not search:
                    cat_id = self.sync_categories(
                        cr, uid, url, session, l, context)
                    if cat_id:
                    ...
                    if map_id.need_sync == 'Yes' and map_id.mag_category_id != -1:
                        cat_update = self._update_specific_category(
                            cr, uid,  map_id.id, url, session, context)
                        if cat_update:
                            if cat_update[0] != 0:
            ...
            if success_exp_ids:
                text = "\nThe Listed category ids %s has been created on magento." % (
                    success_exp_ids)
            if fail_ids:
                text += "\nSelected category ids %s are already synchronized on magento." % (
                    fail_ids)
            if text:
                self.pool.get('magento.sync.history').create(cr, uid, {
...
##########################################################
    ...
    def export_products(self, cr, uid, ids, context=None):
        ...
        pro_op = len(self.pool.get('product.attribute.value').search(
            cr, uid, [], context=context))
        map_op = len(self.pool.get(
            'magento.product.attribute.value').search(cr, uid, []))
        ...
        if connection:
            ...
            context['instance_id'] = instance_id
            server = xmlrpclib.Server(url)
            map_ids = map_tmpl_pool.search(
                cr, uid, [('instance_id', '=', instance_id)])
            for map_id in map_ids:
                map_obj = map_tmpl_pool.browse(cr, uid, map_id)
                synced_ids.append(map_obj.template_name.id)
            template_ids = self.pool.get('product.template').search(
                cr, uid, [('id', 'not in', synced_ids), ('type', '=', 'service')], context=context)
            if not template_ids:
                raise osv.except_osv(_('Information'), _(
                    "No new product(s) Template found to be Sync."))
            if template_ids:
                for template_id in template_ids:
                    prodtype = self.pool.get('product.template').browse(
                        cr, uid, template_id, context=context).type
                    if prodtype == 'service':
                        # error_ids.append(template_id)
                        continue
                    response = self._export_specific_template(
                        cr, uid, template_id, url, session, context)
                    if response:
                        if response[0] > 0:
    ...
    def export_bulk_product_template(self, cr, uid, context=None):
        ...
        if connection:
            ...
            if context.has_key('sync_opr') and context['sync_opr'] == "export":
                for l in bulk_ids:
                    prodtype = self.pool.get('product.template').browse(
                        cr, uid, l, context=context).type
                    if prodtype == 'service':
                        error_ids.append(l)
                        continue
                    search = map_obj.search(cr, uid, [(
                        'template_name', '=', l), ('instance_id', '=', context.get('instance_id'))])
                    if not search:
                        pro = self._export_specific_template(
                            cr, uid, l, url, session, context)
                        if pro:
                            if pro[0] > 0:
                                success_exp_ids.append(l)
                            else:
            ...
            if context.has_key('sync_opr') and context['sync_opr'] == "update":
                ...
                for l in bulk_ids:
                    ...
                    search = map_obj.search(cr, uid, [('template_name', '=', l), (
                        'instance_id', '=', context.get('instance_id')), ('need_sync', '=', 'Yes')])
                    if search:

                        pro_update = self._update_specific_product_template(
                            cr, uid,  search[0], url, session, all_product_data, context)
                        if pro_update:
                            if pro_update[0] > 0:
                ...
                if all_product_data:
                    try:
                        server = xmlrpclib.Server(url)
                        temp = server.call(session, 'magerpsync.magento_product_update', [
                                           all_product_data])
                    except xmlrpclib.Fault, e:
                        pass
            cr.commit()
            if success_exp_ids:
                text = "\nThe Listed Product(s) %s successfully created on Magento." % (
                    success_exp_ids)
            if fail_ids:
                text += "\nSelected product(s) %s are already synchronized on magento." % (
                    fail_ids)
            if error_ids:
        ...
        if id:
            ...
            if obj_pro.product_variant_ids:
                if obj_pro.attribute_line_ids:
                    ...
                    ############# check attributes lines and set attributes are
                    check_attribute = self._check_attribute_with_set(
                        cr, uid, set_id, obj_pro.attribute_line_ids)
                    ...
                    if mage_set_id:
                        for type_obj in obj_pro.attribute_line_ids:
                            ...
                            single_value_dic = {}
                            # fetching options values array for configurable
                            # product
                            for attr_id in obj_pro.attribute_line_ids:
                                ...
                                val_dic = self._search_single_values(
                                    cr, uid, id, attr_id.attribute_id.id, context)
                                if val_dic:
                                    single_value_dic.update(val_dic)
                                for value_id in attr_id.value_ids:
                                    price_extra = 0.0
                                    ##### product template and value extra pric
                                    price_search = val_price_pool.search(
                                        cr, uid, [('product_tmpl_id', '=', id), ('value_id', '=', value_id.id)])
                                    if price_search:
                    ...
                    for obj in obj_pro.product_variant_ids:
                        ...
                        if search_ids:
                            ...
                            if mage_ids[0] > 0:
                                ...
                                map_tmpl_pool.create(cr, uid, {'template_name': id, 'erp_template_id': id, 'mage_product_id': mage_ids[
                                                     1], 'base_price': price, 'is_variants': False, 'instance_id': context.get('instance_id')})
                            return mage_ids
            else:
            ...
            get_product_data = self._get_product_array(
                cr, uid, url, session, obj_pro, get_product_data, context)
            get_product_data['tax_class_id'] = '0'
            if mage_set_id:
                template_sku = 'Template sku %s' % id
                get_product_data['proveedor_'] = obj_pro.preferred_supplier
                get_product_data['stock_data'] = {'manage_stock': 1, 'is_in_stock': 1}
                newprod_data = ['configurable', mage_set_id,
                                template_sku, get_product_data]
                self.pool.get('product.template').write(
                    cr, uid, id, {'prod_type': 'configurable'}, context)
                try:
                    mage_product_id = server.call(
                        session, 'product.create', newprod_data)
                except xmlrpclib.Fault, e:
                    return [0, str(id) + str(e)]
                map_tmpl_pool.create(cr, uid, {'template_name': id, 'erp_template_id': id, 'mage_product_id': mage_product_id, 'base_price': get_product_data[
                                     'price'], 'is_variants': True, 'instance_id': context.get('instance_id')})
                server.call(session, 'magerpsync.template_map', [
                            {'mage_template_id': mage_product_id, 'erp_template_id': id}])
                if mage_product_id:
                    self._create_product_attribute_media(
                        cr, uid, url, session, obj_pro, mage_product_id, id, context)
    ...
    def _get_product_array(self, cr, uid, url, session, obj_pro, get_product_data, context=None):
        ...
        for i in obj_pro.categ_ids:
            mage_categ_id = self.sync_categories(
                cr, uid, url, session, i.id, context=context)
            prod_catg.append(mage_categ_id)
        if obj_pro.categ_id.id:
            mage_categ_id = self.sync_categories(
                cr, uid, url, session, obj_pro.categ_id.id, context=context)
            prod_catg.append(mage_categ_id)
        status = 2
        if obj_pro.sale_ok:
            status = 1
        get_product_data['name'] = obj_pro.name
        get_product_data['cost'] = obj_pro.standard_price
    ...
    def _create_product_attribute_media(self, cr, uid, url, session, obj_pro, mage_product_id, odoo_product_id, context=None):
        if obj_pro.image:
            ...
            if image_file:
                try:
                    server.call(session, 'magerpsync.update_product_image', [
                                [mage_product_id, image_file, "create"]])
                except xmlrpclib.Fault, e:
                    return [0, str(mage_product_id) + str(e)]
                return True
    ...
    def _update_product_attribute_media(self, cr, uid, url, session, obj_pro, mage_product_id, odoo_product_id, context=None):
        if obj_pro.image:
            ...
            try:
                server.call(session, 'magerpsync.update_product_image', [
                            [mage_product_id, image_file, "write"]])
            except xmlrpclib.Fault, e:
                return [0, str(mage_product_id) + str(e)]
            return True
        return False
    ...
    def _export_specific_product(self, cr, uid, id, template_sku, url, session, context=None):
        ...
        if id:
            ...
            if obj_pro.attribute_value_ids:
                ...
                attr_search_ids = mage_attr_pool.search(
                    cr, uid, [('erp_id', '=', pro_attr_id)])
                if attr_search_ids:
                    get_product_data[
                        'currentsetname'] = obj_pro.product_tmpl_id.attribute_set_id.name
            ...
            if obj_pro.type in ['product', 'consu']:
                ...
                prodtype = 'virtual'
            stock = 0
            qty_available = obj_pro.qty_available
            if qty_available > 0:
                stock = 1
            stockData = {'manage_stock': 1, 'qty': qty_available, 'is_in_stock': stock}
            get_product_data['stock_data'] = stockData
            self.pool.get('product.product').write(
                cr, uid, id, {'prod_type': prodtype}, context)
            get_product_data['proveedor_'] = obj_pro.preferred_supplier
            pro = self.prodcreate(cr, uid, url, session,
                                  id, prodtype, sku, get_product_data, context)
            if pro and pro[0] != 0:
                self._create_product_attribute_media(
                    cr, uid, url, session, obj_pro, pro[1], id, context)
            return pro

    #############################################
    ##          single products create         ##
    #############################################
    def prodcreate(self, cr, uid, url, session, pro_id, prodtype, prodsku, put_product_data, context=None):
        context = dict(context or {})
        server = xmlrpclib.Server(url)
        ...
        if pro:
            self.pool.get('magento.product').create(cr, uid, {
                'pro_name': pro_id, 'oe_product_id': pro_id, 'mag_product_id': pro, 'instance_id': context.get('instance_id')})
            server.call(session, 'magerpsync.product_map', [
                        {'mage_product_id': pro, 'erp_product_id': pro_id}])
            return [1, pro]

    # `
    ##          update all products            ##
    #############################################
    def update_products_via_cron(self, cr, uid, context=None):
        context = dict(context or {})
        sucessfully_update = 0
        product_mapping_obj = self.pool.get('magento.product')
        template_mapping_obj = self.pool.get('magento.product.template')
        connection = self.pool.get(
            'magento.configure')._create_connection(cr, uid, context)
        if connection:
            url = connection[0]
            ...
            context['instance_id'] = instance_id
            server = xmlrpclib.Server(url)
            mapping_ids = product_mapping_obj.search(cr, uid, [(
                'pro_name', '!=', False), ('need_sync', '=', 'Yes'), ('instance_id', '=', instance_id)])
            template_mapping_ids = template_mapping_obj.search(
                cr, uid, [('need_sync', '=', 'Yes'), ('instance_id', '=', instance_id)])
            if mapping_ids:
                indexing = False
                update_products_count = 0
                total_count = len(mapping_ids)
                _logger.info(
                    "Total %r Product(s) Ready for update at magento end!!!", total_count)
                for splitted_id in range(0, total_count, 5):
                                        #import pdb; pdb.set_trace()
                    updated_ids = mapping_ids[splitted_id:splitted_id + 5]
                    product_update_data = []
                    update_products_count += len(updated_ids)
                    for mapping_id in updated_ids:
                        mapping_obj = product_mapping_obj.browse(
                            cr, uid, mapping_id, context=context)
                        product_id = mapping_obj.pro_name.id
                        magento_id = mapping_obj.mag_product_id
                        if mapping_obj.pro_name.type == "service":
                            continue
                                                    
                        qty_available = mapping_obj.pro_name.qty_available
                        stock = 0
                        if qty_available > 0:
                            stock = 1
                        stockData = {'manage_stock': 1, 'qty': qty_available, 'is_in_stock': stock}
                        
                        product_update_data.append({
                            'price': mapping_obj.pro_name.list_price,
                            'cost': mapping_obj.pro_name.standard_price,
                            'proveedor_': str(mapping_obj.pro_name.preferred_supplier),
                            'mage_product_id': magento_id,
                            'stock_data': stockData
                        })
                        if update_products_count == total_count:
                            indexing = True
                    try:
                        _logger.info(
                            "Started Updating %r Products  at magento end!!! Indexing = %r ", update_products_count, indexing)
                        _logger.info(
                            "%s Productos que se estan actualizando", product_update_data)
                        server.call(session, 'magerpsync.magento_product_update', [
                                    product_update_data, indexing])
                        _logger.info(
                            "%r Product(s) successfully updated at magento end!!!", update_products_count)
                    except xmlrpclib.Fault, e:
                        _logger.info("Product Update Exception %r", e)
                product_mapping_obj.write(cr, uid, mapping_ids, {
                                          'need_sync': 'No'}, context)
                template_mapping_obj.write(cr, uid, template_mapping_ids, {
                                           'need_sync': 'No'}, context)
        return True
    ...
    def update_products(self, cr, uid, ids, context=None):
        ...
        if connection:
            ...
            context['instance_id'] = instance_id
            server = xmlrpclib.Server(url)
            map_ids = self.pool.get('magento.product.template').search(
                cr, uid, [('need_sync', '=', 'Yes'), ('instance_id', '=', instance_id)])
            pro_ids = self.pool.get('product.template').search(
                cr, uid, [], context=context)
            if not map_ids or not pro_ids:
                raise osv.except_osv(_('Information'), _(
                    "No product(s) Template has been found to be Update on Magento!!!"))
            if map_ids:
                update_products_count = 0
                indexing = False
                for splited_id in range(0, len(map_ids), 10):
                    splited_ids = map_ids[splited_id:splited_id + 10]
                    all_product_data = []
                    update_products_count += len(splited_ids)
                    for i in splited_ids:
                        sucessfully_update = sucessfully_update + 1
                        temp_obj = self.pool.get('magento.product.template').browse(
                            cr, uid, i, context=context)
                        temp_id = temp_obj.template_name.id
                        if temp_id:
                            prodtype_obj = self.pool.get('product.template').browse(
                                cr, uid, temp_id, context=context)
                            prodtype = prodtype_obj.type
                            magento_prodtype = prodtype_obj.prod_type
                            if prodtype == 'service':
                                # up_error_ids.append(temp_id)
                                continue
                        pro_update = self._update_specific_product_template(
                            cr, uid, i, url, session, all_product_data, context=context)
                        if pro_update:
                            if pro_update[0] != 0:
                                ...
                                up_error_ids.append(pro_update[1])
                    if all_product_data:
                        if update_products_count == len(map_ids):
                            indexing = True
                        try:
                            temp = server.call(session, 'magerpsync.magento_product_update', [
                                               all_product_data, indexing])
                        except xmlrpclib.Fault, e:
                            pass
                if success_ids:
                    text = 'The Listed Product ids %s has been sucessfully updated to Magento. \n' % success_ids
                    self.pool.get('magento.sync.history').create(cr, uid, {
                        'status': 'yes', 'action_on': 'product', 'action': 'c', 'error_message': text})
                if up_error_ids:
                    text1 = 'The Listed Product ids %s does not updated on magento.' % up_error_ids
                    self.pool.get('magento.sync.history').create(cr, uid, {
                        'status': 'no', 'action_on': 'product', 'action': 'c', 'error_message': text1})
                text2 = text + text1
                if not text2:
                    raise osv.except_osv(_('Information'), _(
                        "No product(s) has been found to be Update on Magento!!!"))
                partial = self.pool.get('message.wizard').create(
                    cr, uid, {'text': text2})
                return {'name': _("Information"),
                        'view_mode': 'form',
    ...
    #############################################
    def _update_specific_product_template(self, cr, uid, id, url, session, all_product_data, context=None):
        context = dict(context or {})
        get_product_data = {}
        mage_variant_ids = []
        mage_price_changes = {}
        ...
        if temp_id and mage_id:
                        ...
                        for obj in obj_pro.product_variant_ids:
                            ...
                            if search_ids:
                                mage_update_ids = self._update_specific_product(
                                    cr, uid, search_ids[0], url, session, all_product_data, context)
                else:
                    for obj in obj_pro.product_variant_ids:
                        name = obj_pro.name
                        price = obj_pro.list_price or 0.0
                        ...
                        if search_ids:
                            mage_update_ids = self._update_specific_product(
                                cr, uid, search_ids[0], url, session, all_product_data, context=context)
            ...
            get_product_data['proveedor_'] = obj_pro.preferred_supplier
            get_product_data['mage_product_id'] = mage_id
            # update_data = [mage_id, get_product_data]
            all_product_data.append(get_product_data)
    ...
    #############################################
    def _update_specific_product(self, cr, uid, id, url, session, all_product_data, context=None):
        context = dict(context or {})
        server = xmlrpclib.Server(url)
        get_product_data = {}
        ...
        warehouse_id = pro_obj.instance_id.warehouse_id.id
        ...
        if pro_id and mage_id:
            ...
            get_product_data['cost'] = obj_pro.standard_price
            get_product_data['proveedor_'] = obj_pro.preferred_supplier
            total_quantity = obj_pro.qty_available
            stock = 0
            if total_quantity > 0:
                stock = 1
            stockData = {'manage_stock': 1, 'qty': total_quantity, 'is_in_stock': stock}
            get_product_data['stock_data'] = stockData
            get_product_data['mage_product_id'] = mage_id
            # update_data = [mage_id, get_product_data]
            all_product_data.append(get_product_data)
            test = self.pool.get('magento.product').write(
                cr, uid, id, {'need_sync': 'No'}, context)
            return [1, pro_id]
    ...
    def get_mage_region_id(self, cr, uid, url, session, region, country_code, context=None):
        ...
        if not map_id:
            return_id = self.pool.get('region.wizard')._sync_mage_region(
                cr, uid, url, session, country_code)
