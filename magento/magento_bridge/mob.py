...
class magento_website(osv.osv):
    ...
    def _get_website(self, cr, uid, website, context=None):
        ...
        websites = self.search(cr, uid, [('website_id', '=', website[
                               'website_id']), ('instance_id', '=', instance_id)])
        if websites:
...
class magento_store(osv.osv):
    ...
    def _get_store_group(self, cr, uid, group, website, context=None):
        ...
        groups = self.search(cr, uid, [('group_id', '=', group[
                             'group_id']), ('instance_id', '=', instance_id)])
        if groups:
            group_id = groups[0]
        else:
...
class magento_store_view(osv.osv):
    ...
    def _get_store_view(self, cr, uid, store, context=None):
        ...
        views = self.search(cr, uid, [('view_id', '=', store[
                            'store_id']), ('instance_id', '=', instance_id)])
        if views:
            view_id = views[0]
        else:
            group_id = group_pool._get_store_group(
                cr, uid, store['group'], store['website'], context)
            view_dict = {
                'name': store['name'],
                'code': store['code'],
    ...
    def _fetch_magento_store(self, cr, uid, url, session, context=None):
        ...
        for store in stores:
            if store['website']['is_default'] == '1':
                ...
                store_info['store_id'] = view_pool._get_store_view(
                    cr, uid, store, context)
                break
        return store_info
        ...
        if vals:
            if instance_value.instance_name == None or instance_value.instance_name == False:
                vals['instance_name'] = self.pool.get(
                    'ir.sequence').get(cr, uid, 'magento.configure')
    ...
    def _create_connection(self, cr, uid, context=None):
        ...
        if instance_id:
            ...
            try:
                ...
                raise osv.except_osv(_('Error, %s') % e.faultString, _(
