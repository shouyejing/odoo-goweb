            ...
            for id in ids:
                ...
                if not line_ids and vid and image_ids:
                    self.pool.get('product.product').write(
                        cr, uid, vid, {'pro_extra_image': [(6, 0, image_ids)]}, context)
        return res

    def create(self, cr, uid, vals, context=None):
        context = dict(context or {})
        new_id = super(product_template, self).create(
        ...
        if product_id and product_type and mage_product_id:
            for data in image_list:
                if data.has_key('types'):
                    ...
                    type_ids = []
                    for typ in types:
                        search_type = type_pool.search(
                            cr, uid, [('name', '=', typ)])
                        if search_type:
                            type_ids.append(search_type[0])
                        else:
                            type_ids.append(type_pool.create(
                                cr, uid, {'name': typ}))
                    data.pop('types')
                    if type_ids:
                        data['image_type'] = [(6, 0, type_ids)]
                data['mage_product_id'] = mage_product_id
                img_search = self.search(cr, uid, [('mage_file', '=', data.get(
                    'mage_file')), ('mage_product_id', '=', mage_product_id)])
                if not img_search:
            ...
            if img_ids:
                ...
                if product_type == 'product':
                    product_template_id = self.pool.get('product.product').browse(
                        cr, uid, product_id).product_tmpl_id.id
                    line_ids = self.pool.get('product.template').browse(
                        cr, uid, product_template_id).attribute_line_ids.ids
                    if not line_ids and product_template_id:
                        self.pool.get('product.template').write(cr, uid, product_template_id, {
                            'tmpl_extra_image': [(6, 0, img_ids)]}, context)
                    else:
                        self.pool.get('product.product').write(cr, uid, product_id, {
                            'pro_extra_image': [(6, 0, img_ids)]}, context)
                elif product_type == 'template':
                    self.pool.get('product.template').write(cr, uid, product_id, {
                        'tmpl_extra_image': [(6, 0, img_ids)]}, context)
            return True
        return False
...
class magento_synchronization(osv.osv):
    ...
    def _export_product_extra_images(self, cr, uid, id, pro, url, session, context=None):
        ...
        for i in obj_pro.pro_extra_image:
            ...
            pic = {'file': files, 'label': obj_img.name,
                   'position': position, 'types': types, 'exclude': 0}
            image = [pro[1], pic]
            k = server.call(
                session, 'catalog_product_attribute_media.create', image)
            img_pool.write(cr, uid, [i.id], {
                           'mage_file': k, 'mage_product_id': pro[1]})
        return pro

    def _export_specific_product(self, cr, uid, id, template_sku, url, session, context=None):
        pro = super(magento_synchronization, self)._export_specific_product(
            cr, uid, id, template_sku, url, session, context=context)
        if pro and pro[0] != 0:
            self._export_product_extra_images(
                cr, uid, id, pro, url, session, context)
        return pro
    ...
    #   pro = super(magento_synchronization,self)._update_specific_product(cr, uid, id, url, session, all_product_data, context)
    #   if pro and pro[0]:
    #       server = xmlrpclib.Server(url)
