...
class PurchaseOrder(models.Model):
    ...
    def action_picking_create(self):
        ...
        for order in self:
            ...
            document = order.picking_ids
            consignee = document.consignee_id
            partner = document.partner_id
            if partner not in document.message_follower_ids:
                document.message_subscribe([partner.id])
            if consignee not in document.message_follower_ids:
                #self.message_subscribe([consignee.id])
                document.message_subscribe([consignee.id])
                #document.message_subscribe([partner.id])
        return res
