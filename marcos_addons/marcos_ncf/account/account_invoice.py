...
class account_invoice(models.Model):
    ...
    def _invoice_ncf_validate(self):
        if self.type in ['in_invoice', "in_refund"] and not self.reference_type in ['none', 'ext'] and not self.journal_id.ncf_special in ['gasto', 'informal']:
    ...
    def create(self, vals):
        ...
        if self.env.context.get("type", False) == 'out_invoice':
            ...
            if not vals.get("fiscal_position", False):
                vals.update(
                    {
                    "fiscal_position": self.env['res.partner'].browse(vals["partner_id"]).property_account_position.id})
    ...
    def write(self, vals):
        if self.internal_number and not self.number:
            ...
            if vals.get("internal_number", False):
                ...
                self.env.cr.execute("select id from account_invoice where number = %(number)s and partner_id = %(partner_id)s",
    ...
    def invoice_validate(self):
        ...
        if self.type in ["in_invoice", "in_refund"] and self.reference_type == "none":
            raise except_orm("Advertencia!", u'No puede validar una factura de compra con el tipo pendiente de digitar!')
...
class account_invoice_line(models.Model):
    ...
    def product_id_change(self, product, uom_id, qty=0, name='', type='out_invoice',
        ...
        if product:
            name = \
                self.pool.get("product.product").name_get(self.env.cr, self.env.uid, [product],
                                                          context=self.env.context)[
                    0][1]
            res["value"]["name"] = name
        return res
